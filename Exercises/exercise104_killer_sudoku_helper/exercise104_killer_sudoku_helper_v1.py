...
from itertools import filterfalse as ffalse, combinations as combi
...
def combinations(target, size, exclude):
    ...
    return list(list(item) for item in ffalse(lambda element: sum(element) != target,
           combi(ffalse(lambda digit: digit in exclude, range(1, 10)), size)))
